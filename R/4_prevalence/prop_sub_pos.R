...
data = escalc(measure = "PLO", xi = sub_pos, mi = sub_neg, data = data, append = TRUE)

mod_prop_sub_pos = rma.uni(measure = "PLO", xi = sub_pos, mi = sub_neg, data = data, slab = survey_year)

# Extract parameters for summary value 

y = mod_prop_sub_pos$b[1]
se = (predict(mod_prop_sub_pos)$cr.ub - y)/1.96
...
write.csv(output, "interim_outputs/prop_sub_pos_summ.csv", row.names = FALSE)
...
write.csv(data, "interim_outputs/prop_sub_pos_all.csv", row.names = FALSE)
